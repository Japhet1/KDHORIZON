import { AiOutlineLine } from "react-icons/ai";
import AgricGallery from "./AgricGallery";


const AgricService = () => {
  return (
    <main className=" pb-20">
        <div className="text-center items-center text-black font-leelawadee p-10 mb-20">
          ` <p className="text-3xl md:px-10 lg:px-40 leading-10">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                et des vergers abondants.
            </p>
        </div>

        <div className="flex flex-wrap justify-center items-center lg:gap-10  md:text-left lg:text-left">
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                    et des vergers abondants.
                </p>
            </div>
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                et des vergers abondants.
                </p>
            </div>
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                    et des vergers abondants.
                </p>
            </div>
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                    et des vergers abondants.
                </p>
            </div>
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                    et des vergers abondants.
                </p>
            </div>
            <div className="md:max-w-xs lg:max-w-md p-6">
                <div className=" my-5">
                    <h1 className="text-black text-3xl font-bold font-gruppo">Service</h1>
                    <AiOutlineLine className="w-full md:w-auto lg:w-auto text-3xl text-green-700 text-center"/>
                </div>
                <p className="leading-8 text-lg font-leelawadee text-gray-700">Nichée au cœur dune campagne fertile, notre ferme possède des champs étendus
                    et des vergers abondants.
                </p>
            </div>
            
        </div>

        

        <AgricGallery />
    </main>
  )
}

export default AgricService